...
import threading
from repositories.roadmap_repository import load_raw_career_roadmaps, clear_cache as repo_clear_cache

_validated_roadmaps_cache = None
...
def load_all_career_roadmaps():
    """Load, validate, and return all career roadmaps (cached)."""
    global _validated_roadmaps_cache
    if _validated_roadmaps_cache is None:
        with _service_lock:
            if _validated_roadmaps_cache is None:
                roadmaps = load_raw_career_roadmaps()
                validate_roadmaps(roadmaps)
                _validated_roadmaps_cache = roadmaps
    return _validated_roadmaps_cache
